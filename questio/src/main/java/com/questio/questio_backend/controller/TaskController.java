...
public class TaskController {
    ...
    @GetMapping("/turmas/{idTurma}/desempenho")
    @PreAuthorize("hasAnyRole('PROFESSOR', 'COORDENACAO')")
    public ResponseEntity<List<DesempenhoAlunoDTO>> desempenhoPorTurma(@PathVariable UUID idTurma) {
        return ResponseEntity.ok(tarefaService.desempenhoPorTurma(idTurma));
    }
    @GetMapping("/turmas")
    @PreAuthorize("hasAnyRole('PROFESSOR', 'COORDENACAO')")
    public ResponseEntity<List<ClassResponseDTO>> listarTurmasDoProfessor() {
        return ResponseEntity.ok(tarefaService.listarTurmasDoProfessor());
    }

    @PatchMapping("/{id}/avaliar")
    @PreAuthorize("hasAnyRole('PROFESSOR', 'COORDENACAO')")
    public ResponseEntity<Void> avaliar(@PathVariable UUID id, @RequestBody AvaliacaoDTO dto) {
        tarefaService.avaliarTarefa(id, dto);
